    ...
    public class FurnaceTileBehaviour : BreakableTileBehaviour
    {
        ...
        private void LoadRecipeFromMetadata()
        {
            ...
            if (metadata != null && metadata.ContainsKey("recipeOutput"))
            {
                ...
                if (gameState != null)
                {
                    currentRecipe = gameState.itemRecipes.Find(r => r.outputItemName == recipeName && r.type == RecipeType.Smelting);
                }
            }
        }
        ...
        void Update()
        {
            ...
            if (mouseDown && isHovered && !furnaceUI.activeSelf)
            {
                ...
                script.ClearListeners(); // Clear previous listeners
                script.SelectRecipe(currentRecipe);
                script.selectRecipeEvent.AddListener(OnSelectRecipe);
                script.currentFurnace = this; // Set reference to current furnace
